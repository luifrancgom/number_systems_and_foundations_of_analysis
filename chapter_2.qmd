...
Second, assume $y \in B$. Then, $\mu(x,y) = \tau(x,y)$. But, $\tau(x,S(y)) = \tau(x,y) \times x = \mu(x,y) \times x = \mu(x,S(y))$. This $S(y) \in B$, and we have shown that $y \in B \Longrightarrow S(y) \in B$. Hence, by matjematical induction, $\mathbb{P} = B$. Thus, $\mu(x,y) = \tau(x,y)$ for all $x,y \in \mathbb{P}$. This means that $\mu = \tau$.
