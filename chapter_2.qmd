...
::: {.proof}
a. Consider $x, y \in \mathbb{P}$. Let
   
   $$B = \{ z: z \in \mathbb{P} \land x \times (y + z) = (x \times y) + (x \times z) \}$$

   We must prove that $B = \mathbb{P}$. First, $1 \in B$, since:

   $\begin{aligned}
     x \times (y + 1) & = x \times S(y) \\
     & = x \times S(y) \\
     & = (x \times y) + x \\
     & = (x \times y) + (x \times 1) 
     \end{aligned}$

   Now assume $z \in B$. Then $z \in \mathbb{P}$ and $x \times (y + z) = (x \times y) + (x \times z)$. Therefore:
   ...
   Thus, $S(z) \in B$. Therefore because $x \in B \Longrightarrow S(x) \in B$ then $B = \mathbb{P}$.

b. Consider $y, z \in \mathbb{P}$. Let
   
   $$B = \{ x: x \in \mathbb{P} \land (y + z) \times x = (y \times x) + (z \times x) \}$$
   ...
   Now assume $x \in B$. Then $x \in \mathbb{P}$ and $(y + z) \times x = (y \times x) + (z \times x)$. Therefore:

   $\begin{aligned}
     (y + z) \times S(x) & = ((y + z) \times x) + (y + z) \\
     & = ((y \times x) + (z \times x)) + (y + z) \\
     & = (y \times x) + ((z \times x) + (y + z)) \\
     & = (y \times x) + ((z \times x) + (z + y)) \\
     & = (y \times x) + (((z \times x) + z) + y) \\
     & = (y \times x) + (z \times S(x) + y) \\
     & = (y \times x) + (y + z \times S(x)) \\
     & = ((y \times x) + y) + z \times S(x) \\
     & = y \times S(x) + z \times S(x) \\
     \end{aligned}$
    
   Thus, $S(x) \in B$. Therefore because $x \in B \Longrightarrow S(x) \in B$ then $B = \mathbb{P}$.
:::

If we already knew that multiplication is commutative. @thm-distributive-laws b. would be a consequence of @thm-distributive-laws a. However, we still be done in @thm-commutativity-of-multiplication and the proof will depend upon @thm-distributive-laws b.

::: {#exr-distributive-laws}
1. Prove $2 \times x = x + x$

2. Prove $3 \times x = (x + x) + x$
:::

::: {#sol-distributive-laws}
1. Using @lem-commutativity-of-multiplication:
...
2. Using @lem-commutativity-of-multiplication and @exr-distributive-laws 1.:
...
::: {#lem-commutativity-of-multiplication}
$1 \times x = x \times 1$ for all $x \in P$
:::
...
$$
B = \{ x: x \in \mathbb{P} \land 1 \times x = x \times 1 \}
$$

We must prove that $B = \mathbb{P}$. First, $1 \in B$, since $1 \times 1 = 1 \times 1$.

Now assume $x \in B$. Then $x \in \mathbb{P}$ and $1 \times x = x \times 1$. Therefore:

$\begin{aligned}
  1 \times S(x) & = 1 \times (x + 1) \\
  & = 1 \times x + 1 \times 1 \\
  & = x \times 1 + 1 \times 1 \\
  & = (x + 1) \times 1 \\
  & = S(x) \times 1
...
::: {#thm-commutativity-of-multiplication}
## Commutativity of multiplication

$$x \times y = y \times x$$

For all $x, y \in \mathbb{P}$.
:::

::: {.proof}
Consider $x \in \mathbb{P}$. Let
   
   $$B = \{ y: y \in \mathbb{P} \land x \times y = y \times x \}$$

We must prove that $B = \mathbb{P}$. First, $1 \in B$, since $x \times 1 = 1 \times x$ by @lem-commutativity-of-multiplication

Now assume $x \in B$. Then $x \in \mathbb{P}$ and $x \times y = y \times x$. Therefore:

$$
\begin{aligned}
 x \times S(y) & = x \times (y + 1) \\
 & = (x \times y) + (x \times 1) \\
 & = (y \times x) + (1 \times x) \\
 & = (y + 1) \times x \\
 & = S(y) \times x
...
::: {#thm-associativity-of-multiplication}
...
$$
B = \{ z: z \in \mathbb{P} \land x \times (y \times z) = (x \times y) \times z \}
$$
We must prove that $B = \mathbb{P}$. First, $1 \in B$, since:

$\begin{aligned}
  x \times (y \times 1) & x \times y \\
  = & (x \times y) \times 1 \\
  \end{aligned}$

Now assume $z \in B$. Then $z \in \mathbb{P}$ and $x \times (y \times z) = (x \times y) \times z$. Therefore:

$\begin{aligned}
  x \times (y \times S(z)) & = x \times ((y \times z) + y) \\
  & = x \times ((y \times z) + y) \\
...
::: {#exr-associativity-of-multiplication}
1. Prove $(x \times y) \times (u \times v) = (x \times u) \times (y \times v)$
:::
...
::: {#thm-order-relation-cancelation-law}
$$
x < y \iff x \times z < y \times z
$$
...
$\begin{aligned}
  x < y & \iff y = x + w \\
  & \Longrightarrow  y \times z = (x + w) \times z \\
  & \iff y \times z = (x \times z) + (w \times z) \\
  & \iff x \times z < y \times z
  \end{aligned}$

Now, assume $x \times z < y \times z$. Lets proof using @nte-proof-by-reductio-ad-absurdum. Assume $x \not< y$ then by @thm-order-relation-natural-numbers-properties-1 c. $x = y$ or $y < x$. If $x = y$ then $x \times z = y \times z$ which contradicts $x \times z < y \times z$. Also if $y < x$ then by the first part $y \times z < x \times z$ which contradicts $x \times z < y \times z$. In any case there is a contradiction so it must be the case that if $x \times z < y \times z$ then $x < y$. 
:::

::: {#cor-cancellation-law-of-multiplication}
...
**Note**: The other direction follows from **Appendix A Assumption II**. That is,

$$
x = y \Longrightarrow x \times z = y \times z
$$
:::

::: {.proof}
Assume $x \times z = y \times z$. Lets proof using @nte-proof-by-reductio-ad-absurdum. Assume $x \neq y$ then by @thm-order-relation-natural-numbers-properties-1 c. $x < y$ or $y < x$. If $x < y$ then by @thm-order-relation-cancelation-law $x \times z < y \times z$ which contradicts $x \times z = y \times z$. Also if $y < x$ then again by @thm-order-relation-cancelation-law $y \times z < x \times z$ which contradicts again $x \times z = y \times z$. Therefore it must be the case that $x = y$.
:::

::: {#cor-order-relation-multiplication}
...
d. $[x < u \land y < v] \Longrightarrow x \times y < u \times  v$

e. $[x < u \land y \leq v] \Longrightarrow x \times y < u \times  v$

f. $[x \leq u \land y < v] \Longrightarrow x \times y < u \times  v$

g. $[x \leq u \land y \leq v] \Longrightarrow x \times y \leq u \times  v$
:::

::: {.proof}
a. By @thm-order-relation-cancelation-law and @cor-cancellation-law-of-multiplication.

b. Let $1 \leq y$ by @exr-order-relation-natural-numbers-2 1. Then for $x = 1$ by @cor-order-relation-multiplication:
...
c. For $x = 1$ by @thm-order-relation-cancelation-law:
...
d. Let $x < u$ and $y < v$ then:

   $\begin{aligned}
     x < u \land y < v & \iff x \times y < u \times y \land y \times u < v \times u \\
     & \iff x \times y < u \times y \land u \times y < u \times v \\
     & \Longrightarrow x \times y < u \times v
     \end{aligned}$   

e. If $y = v$ and $x < u$ then by @thm-order-relation-cancelation-law $x \times y < u \times v$. Also, if $y < v$ and $x < u$ then by @cor-order-relation-multiplication d $x \times y < u \times v$.   

f. If $x = u$ and $y < v$ then by @thm-order-relation-cancelation-law $y \times x < v \times u$. Then by @thm-commutativity-of-multiplication $x \times y < u \times v$. Also, if $x < u$ and $y < v$ then by @cor-order-relation-multiplication d $x \times y < u \times v$.

g. If $x = u \land y \leq v$ or $x \leq u \land y = v$ then by @cor-order-relation-multiplication e or f $x \times y < u \times v$. If $x = u \land y = v$ then by **Appendix A Assumption II** $x \times y = u \times v$. Therefore we have that, $x \times y < u \times v$ or $x \times y = u \times v$ so $x \times y \leq u \times v$.   
:::

::: {#exr-multiplication}
1. We use the standard notation: $x^2$ for $x \times x$, and $x^3$ for $x^2 \times x$,

   a. Prove $1^2 = 1$
   b. Prove $(x + y)^2 = x^2 + (2 \times x \times y) + y^2$

2. Define:
   
   $x$ is even is an abbreviation for $(\exists u)(x = 2 \times u)$

   $x$ is odd is an abbreviation for $x = 1 \lor (\exists u)(x = (2 \times u) + 1)$

   Prove:
   
   i. Every element of $\mathbb{P}$ is either even or odd.
   
   ii. No element of $\mathbb{P}$ is both even and odd.
   
   iii. $x^{2}$ is even $\iff x$ is even.
   
   iv. $x^{2}$ is odd $\iff x$ is odd.
   
   v. The sum and product of even elements is even.
   
   vi. The product of odd elements is odd.
   
   vii. The sum of two odd elements is even.
   
   viii. If $x$ is odd and $y$ is even, then $x + y$ is odd and $x \times y$ is even.
:::

::: {#sol-multiplication}
1. Let $x^2$ for $x \times x$, and $x^3$ for $x^2 \times x$ then:
   ...
   b. $\begin{aligned}
       (x + y)^2 & = (x + y) \times (x + y) \\
       & = ((x + y) \times x) + ((x + y) \times y) \\
       & = ((x \times x) + (y \times x)) + ((x \times y) + (y \times y)) \\
       & = (x^2 + (x \times y)) + ((x \times y) + y^2) \\
       & = ((x^2 + (x \times y)) + (x \times y)) + y^2 \\
       & = (x^2 + ((x \times y) + (x \times y))) + y^2 \\
       & = (x^2 + ((x \times y) \times 1 + (x \times y))) + y^2 \\
       & = (x^2 + (x \times y) \times S(1)) + y^2 \\
       & = (x^2 + (x \times y) \times 2) + y^2 \\
       & = (x^2 + 2 \times (x \times y)) + y^2 \\
       \end{aligned}$ 

2. Taking into account the definitons of even and odd we have that:

   i. Let $B = \{ x: x \in \mathbb{P} \land (x \text{ is even } \lor x \text{ is odd})\}$

      $1 \in B$ because $1$ is odd so $1 \in B$. Now assume $x \in B$ so $x$ is even or $x$ is odd.

      Therefore, if $x$ is even then $x = 2 \times u$ so $x + 1 = S(x) = 2 \times u + 1$. So, $S(x)$ is odd and $S(x) \in B$.

      Now assume $x$ is odd. If $x = 1$ then $x + 1 = S(1) = 1 + 1 = 2 = 2 \times 1$ therefore $S(x)$ is even and $S(x) \in B$. If $x = 2u + 1$ then:
      ...
      So, $S(x)$ is also even and $S(x) \in B$.

      Therefore because $x \in B \Longrightarrow S(x) \in B$ then $B = \mathbb{P}$.
   
   ii. Assume $x$ is odd and even then $x = 1$ and $x = 2u$ or $x = 2w + 1$ and $x = 2u$

       If $x = 1$ and $x = 2 \times u = u \times 2$ then $1 < 2$ and $2 \leq 2 \times u$. Therefore, because $1 < 2$ and $2 \leq 2 \times u$ then $1 < 2 \times u$. So it is not the case that $x = 1$ and $x = 2 \times u$.

       If $x = 2w + 1$ and $x = 2u$ then $2w + 1 = 2u$. This means that $2w < 2u$ and $w < u$. Therefore $u = w + z$. Then $2u = 2(w + z) = 2w + 2z$. Also we have that $2w + 1 = 2w + 2z$ so $1 = 2z$. But by the first part of the proof this is not possible.

       Therefore we have that it is not the case that $x$ is odd and even.

   iii. Assume $x$ is even then $x = 2u$. This means that $x^2 = (2u) \times x = 2 \times (u \times x)$. Therefore $x^2$ is even.

        Now assume $x^2$ is even and $x$ is odd then by @exr-multiplication iv. $x^2$ is odd. However by @exr-multiplication ii. this is not possible. Therefore, it must be the case that $x$ is also even.

   iv. Assume $x$ is odd then $x = 2u + 1$. This means that $x^2 = (2u + 1)^2 = 4u^2 + 2u + 1 = 2(2u^2 + u) + 1$. Therefore $x^2$ is odd.

       Now assume $x^2$ is odd and $x$ is even then by @exr-multiplication iii. $x^2$ is even. However by @exr-multiplication ii. this is not possible. Therefore, it must be the case that $x$ is also odd.

   v. Let $x = 2u$ and $y = 2w$ then

      $\begin{aligned}
       x + y & = 2 \times u + 2 \times w \\
       & = 2 \times (u + w)
       \end{aligned}$
      ...
      $\begin{aligned}
       x \times y & = (2 \times u) \times (2 \times w) \\
       & = 2 \times (u \times (2 \times w))
       \end{aligned}$

      So $x \times y$ is even.

   vi. Let $x = 2u + 1$ and $y = 2w + 1$ then

       $\begin{aligned}
        x \times y & = (2u + 1) \times (2w + 1) \\
        & = (2u + 1) \times 2w + (2u + 1) \times 1 \\
        & = ((2u \times 2w) + (1 \times 2w))  + (2u + 1) \\
        & = ((2u \times 2w) + 2w)  + (2u + 1) \\
        & = (2 \times (u \times 2w) + 2 \times w)  + (2u + 1) \\
        & = 2 \times ((u \times 2w) + w)  + (2u + 1) \\
        & = (2 \times ((u \times 2w) + w)  + 2 \times u) + 1 \\
        & = 2 \times (((u \times 2w) + w)  + u) + 1
        \end{aligned}$

       So $x \times y$ is odd.

   vii. Let $x = 2u + 1$ and $y = 2w + 1$ then
...
::: {#thm-iteration-theorem-exponentiation}
## Exponentiation

Consider any Peano system $(\mathbb{P}, S, 1)$. Then there is a unique binary operation $\tau(x, y)$ on $\mathbb{P}$ such that:

$(\varrho)$. $\tau(x, 1) = x$ for all $x \in \mathbb{P}$

$(\sigma)$. $\tau(x, S(y)) = \tau(x, y) \times x$ for all $x, y \in \mathbb{P}$

**Note**: We shall let $x^y$ stand for $\tau(x, y)$. Then $(\varrho)$, $(\sigma)$ become the familiar equation:

$(\gamma')$. $x^1 = x$

$(\sigma')$. $x^{y + 1} = x^y \times x$
:::

::: {.proof}
To prove the existence take any $x \in \mathbb{P}$. Now apply @thm-iteration-theorem with $W = \mathbb{P}$, $c = x$ and $g(u) = u \times x$ for any $u \in \mathbb{P}$. Therefore, there is a unique function $F: \mathbb{P} \longrightarrow \mathbb{P}$ such that:

a. $F(1) = c = x$

b. $F(S(y)) = g(F(y)) = F(y) \times x$ for any $y \in \mathbb{P}$

We denote this unique function $F$ by $h_x$, since it is determined by $x$. Thus, $h_x(1) = x$ and $h_x(S(y)) = h_x(y) \times x$. Now, define $\tau(x,y) = h_x(y)$. Then, $\tau(x,1) = = h_x(1) = 1$, and $\tau(x,S(y)) = h_x(S(y)) = h_x(y) \times x = \tau(x,y) \times x$. Therefore:

a. $\tau(x,1) = x$ for any $x \in \mathbb{P}$

b. $\tau(x,S(y)) = \tau(x,y) \times x$ for any $x,y \in \mathbb{P}$

To prove the uniqueness of $\tau$, assume that there is another binary operation $\mu$ such that:

$$
\mu(x,1) = x \text{ for all } x \in \mathbb{P}
$$

$$
\mu(x,S(y)) = \mu(x,y) \times x \text{ for all } x,y \in \mathbb{P} 
$$

Take any $x \in \mathbb{P}$. Let us prove that $\mu(x,y) = \tau(x,y)$, for all $y \in \mathbb{P}$. Let $B = \{ y: y \in \mathbb{P} \land \mu(x,y) = \tau(x,y) \}$.

First, $1 \in B$, since $\mu(x, 1) = 1 = \tau(x,1)$

Second, assume $y \in B$. Then, $\mu(x,y) = \tau(x,y)$. But, $\tau(x,S(y)) = \tau(x,y) \times x = \mu(x,y) \times x = \mu(x,S(y))$. This $S(y) \in B$, and we have shown that $y \in B \Longrightarrow S(y) \in B$. Hence, by mathematical induction, $\mathbb{P} = B$. Thus, $\mu(x,y) = \tau(x,y)$ for all $x,y \in \mathbb{P}$. This means that $\mu = \tau$.

**Note**: Our notation for exponentiation does not conflict with out previous agreement that $x^2$ stands for $x \times x$, and $x^3$ for $x^2 \times x$. For according to equation $(\varrho')$, $(\sigma')$, $x^2 = x^1 \times x = x \times x$, and $x^3 = x^{2+1} = x^2 \times x$. 
:::

::: {#exr-exponentiation-1}
1. Compute the values of $2^2$, $2^3$ and $3^2$
:::

::: {#sol-exponentiation-1}
1. Let compute each value:
...
$$
B = \{ x: x \in \mathbb{P} \land 1^x = 1 \}
$$

We must prove that $B = \mathbb{P}$. First, $1 \in B$, since $x \times 1^1 = 1$ by equation $(\varrho')$

Now assume $x \in B$. Then $x \in \mathbb{P}$ and $1^x = 1$. Therefore:
...
::: {#thm-basic-properties-of-exponentiation}
## Basic properties of exponentiation

a. $x^{y + z} = x^y \times x^z$
...
a. Consider $x, y \in \mathbb{P}$. Let
   ...
   $$
   B = \{ z: z \in \mathbb{P} \land x^{y + z} = x^y \times x^z \}
   $$
   We must prove that $B = \mathbb{P}$. First, $1 \in B$, since $x^{y + 1} = x^y \times x = x^y \times x^1$.

   Now assume $z \in B$. Then $z \in \mathbb{P}$ and $x^{y + z} = x^y \times x^z$. Therefore:
...
b. Consider $x, y \in \mathbb{P}$. Let
   
   $$
   B = \{ z: z \in \mathbb{P} \land (x^y)^z = x^{y \times z} \}
   $$
   We must prove that $B = \mathbb{P}$. First, $1 \in B$, since $(x^y)^1 = x^y = x^{y \times 1}$.

   Now assume $z \in B$. Then $z \in \mathbb{P}$ and $(x^y)^z = x^{y \times z}$. Therefore:
...
c. Consider $x, y \in \mathbb{P}$. Let
   
   $$
   B = \{ z: z \in \mathbb{P} \land (x \times y)^z = x^z \times y^z \}
   $$
   We must prove that $B = \mathbb{P}$. First, $1 \in B$, since $(x \times y)^1 = x \times y = x^1 \times y^1$.

   Now assume $z \in B$. Then $z \in \mathbb{P}$ and $(x \times y)^z = x^z \times y^z$. Therefore:

   $$
   \begin{aligned}
    (x \times y)^{S(z)} & = (x \times y)^{z + 1} \\
    & = (x \times y)^z \times (x \times y) \\
    & = (x^z \times y^z) \times (x \times y) \\
    & = (x^z \times y^z) \times (y \times x) \\
    & = ((x^z \times y^z) \times y) \times x \\
    & = (x^z \times (y^z \times y)) \times x \\
    & = (x^z \times y^{z + 1}) \times x \\
    & = (y^{z + 1} \times x^z) \times x \\
    & = y^{z + 1} \times (x^z \times x) \\
    & = y^{z + 1} \times x^{z + 1}  \\
    & = x^{z + 1} \times y^{z + 1} \\
    & = x^{S(z)} \times y^{S(z)}
...
::: {#thm-exponentiation-and-the-order-relation}
## Exponentiation and the order relation

For any $x, y, z \in \mathbb{P}$:

a. $x < y \iff x^z < y^z$ 

b. $(1 < z \land x < y) \iff z^x < z^y$

c. $1 < x \iff z < x^z$

d. $(1 < z \land 1 < y) \iff z < z^y$
:::

::: {.proof}
a. Lets divide the proof in 2 parts:

   First, assume $x < y$. Let $x, y \in \mathbb{P}$ and $B = \{ z: z \in \mathbb{P} \land x^z < y^z  \}$. Then we must show that $B = \mathbb{P}$.
   
   We have that $1 \in B$. Because $x^1 = x < y = y^1$. 
   
   Now, assume $z \in B$. Then, $x^z < y^z$. Therefore:
   ...
   \end{aligned}
   $$
   Where by @cor-order-relation-multiplication d. $x < y \land x^z < y^z$ then $x^z \times x < y^z \times y$. Hence, by mathematical induction, $\mathbb{P} = B$.

   Second, assume $x^z < y^z$. Also assume that $x \not< y$. Then $x = y$ or $y < x$. If $x = z$ then $x^z = y^z$ but $x^z < y^z$. Now, if $y < x$ then by the first part of the proof $y^z < x^z$ but $x^z < y^z$. So it must be the case that if $x^z < y^z$ then $x < y$.

b. Lets divide the proof in 2 parts:

   First, assume $1 < z \land x < y$. Then $1^u = 1 < z^u$ by @lem-multiplication and @thm-exponentiation-and-the-order-relation a. Also $y = x + u$. 
   
   Also by @thm-order-relation-cancelation-law, if $1 < z^u$ then $1 \times z^x < z^u \times z^x$. Therefore:
   ...
   Where $z^x \times z^u = z^{x + u}$ by @thm-basic-properties-of-exponentiation a.

   Second, assume $z^x < z^y$. It must be the case that $z \neq 1$ because by @lem-multiplication $1^x = 1 = 1^y$. Therefore by @thm-order-relation-natural-numbers-extent-2 a. $1 < z$.

   Now assume that $x \not< y$. Then by @thm-order-relation-natural-numbers-properties-1 $x = y$ or $y < x$. If $x = y$ then $z^x = z^y$ but $z^x < z^y$. Also if $y < x$ by the first part of the proof $$z^y < z^x$ but again $z^x < z^y$. Therefore, it must be the case that $x < y$.  

c. Lets divide the proof in 2 parts:

   First, assume $1 < x$. Let $x \in \mathbb{P}$ and $B = \{ z: z \in \mathbb{P} \land z < x^z  \}$. Then we must show that $B = \mathbb{P}$.
   
   We have that $1 \in B$. Because $1 < x^1 = x$ where it is assumed that $1 < 1$. 
   
   Now, assume $z \in B$. Then, $z < x^z$. Therefore we have that $z < x \times z = z \times x$ by @cor-order-relation-multiplication. Also if $z < x^z$ by @thm-order-relation-cancelation-law $z \times x < x^z \times x$. Therefore:

   $$
   \begin{aligned}
   z < x^z & \iff z \times x < x^z \times z \\
   & \iff z < z \times x \land z \times x < x^z \times z \\
   & \Longrightarrow z < x^z \times z \\
   & \iff z < x^{z + 1} \\
   & \iff z < x^{S(z)} \\   
   \end{aligned}
   $$

   Hence, by mathematical induction, $\mathbb{P} = B$.

   Second, lets prove the converse. That is, $z < x^z \Longleftarrow 1 < x$. This is equivalent to prove $\neg(z < x^z \land 1 \not< x)$. 
   
   Assume $(z < x^z \land 1 \not< x)$. This means that $x \leq 1$ but by @exr-order-relation-natural-numbers-2 1. $1 \leq x$ so if $1 \not< x$ it must be the case that $x = 1$. However, it must be the case that $z < 1^x = 1$. However by @exr-order-relation-natural-numbers-2 1. this is not possible. Therefore, it must be the case that $\neg(z < x^z \land 1 \not< x)$, which means that $z < x^z \Longleftarrow 1 < x$.   

d. By @thm-exponentiation-and-the-order-relation let $x = 1$ then $(1 < z \land 1 < y) \iff z^1 < z^y$. Therefore, $(1 < z \land 1 < y) \iff z < z^y$. 
:::

::: {#exr-exponentiation-2}
1. Prove $1 < x \iff 1 < x^z$

2. Prove $x \leq x^z$

3. Prove $x \leq y \iff x^z \leq y^z$

4. Prove $x \leq y \Longrightarrow z^{x} \leq z^{y}$

5. Prove $(1 < z \land z^x \leq z^y) \Longrightarrow x \leq y$

6. Prove $(1 + z)^y \geq 1 + (y \times z)$
:::

::: {#sol-exponentiation-2}
1. By @thm-exponentiation-and-the-order-relation we can apply $1 < x \iff 1^z < x^z$. Also by @lem-multiplication $1^z = 1$, so $1 < x \iff 1 < x^z$. 

2. Let $x \in \mathbb{P}$.

   If $z = 1$ then $x \leq x^1 = x$. 
   
   Now if $z > 1$ then $z = S(y) = y + 1$. By @exr-order-relation-natural-numbers-2 1. $1 \leq x^y$. Also by @cor-order-relation-multiplication a. $1 \times x \leq x^y \times x$. Therefore $x \leq x^{y + 1} = x^z$.  

3. If $x < y$ or $x^z < y^z$ it is possible to apply @thm-exponentiation-and-the-order-relation a.

   Now, if $x = y$ then it must be the case that $x^z = y^z$.

   Also if $x^z = y^z$ it is possible to prove the converse by contradiction. Let assume $x^z = y^z$ and $x \neq y$. Then $x < y$ or $x < y$ but by @thm-exponentiation-and-the-order-relation a. $x^z < y^z$ or $x^z < y^z$ which it is a contradiction because $x^z = y^z$. Therefore, it must be the case that $x = y$. 

4. If $x < y$ and $1 < z$ it is possible to apply @thm-exponentiation-and-the-order-relation b.

   If $x < y$ and $1 = z$ then $1^x = 1 = 1^y$ by @lem-multiplication.

   However the converse it not true in the case of $z = 1$ because $1^x = 1 = 1^y$. So it is possible to have $y < x$. 

5. If $z^x < z^y$ then it is possible to apply @thm-exponentiation-and-the-order-relation b. to obtain $x < y$. Also we don't have any contradiction because we obtain $1 < z$.

   Now, assume $z^x = z^y$ and $1 < z$. Assume by contradiction that $x \not \leq y$ or equivalently $y < x$. Then by @thm-exponentiation-and-the-order-relation b. $z^y < z^x$ but $z^x = z^y$. Therefore, it must by the case that $x \leq y$.

6. Let $z \in \mathbb{P}$ and $B = \{ y: y \in \mathbb{P} \land y \times z + 1 \leq (1 + z)^y \}$. Then we must show that $B = \mathbb{P}$.

   We have that $1 \in B$ becasue $1 \times z + 1 = z + 1 = (1 + z)^1 = 1 + z = z + 1$.

   Now asume $y \in B$ so $y \times z + 1 \leq (1 + z)^y$. Therefore:

   $$
   \begin{aligned}
   y \times z + 1 < (1 + z)^y & \iff (y \times z + 1) \times (1 + z) \leq (1 + z)^y \times (1 + z) \\
   & \iff (y \times z + 1) +  (y \times z + 1) \times z \leq (1 + z)^{y + 1} \\
   & \iff (y \times z + 1) +  ((y \times z) \times z + z) \leq (1 + z)^{S(y)} \\
   & \iff ((z \times y + z) + 1) +  (y \times z) \times z \leq (1 + z)^{S(y)} \\
   & \iff (z \times S(y) + 1) +  (y \times z) \times z \leq (1 + z)^{S(y)} \\
   & \Longrightarrow z \times S(y) + 1 < (1 + z)^{S(y)} \\
   \end{aligned}
   $$

   Where $(z \times S(y) + 1) < (z \times S(y) + 1) +  (y \times z) \times z$. So, if $(z \times S(y) + 1) +  (y \times z) \times z \leq (1 + z)^{S(y)}$ then by @thm-order-relation-natural-numbers-extent-1 b. we have that $z \times S(y) + 1 \leq (1 + z)^{S(y)}$.

   Hence, $B = \mathbb{P}$ by the principle of mathematical induction.

   This result is known as the **Bernoulli's inequality**.  
:::

## Isomorphism - Categoricity

Do the axioms for Peano systems completely characterize the natural numbers? The meaning of this question is not quite clear, but we shall give a positive answert in the following sense:

> Any Peano system is essentially the same as the natural numbers

Of course, a precise definition is required for the notion that 2 Peano systems are essentially the same. This leads to the important idea of isomorphic Peano systems.

::: {#def-isomorphic}
Consider any two Peano systems $\mathscr{P} = (\mathbb{P}, 1, S)$ and $\mathscr{P}^* = (\mathbb{P}^*, 1^*, S^*)$. We say that $\mathscr{P}$ and $\mathscr{P}^*$ are isomorphic if and only if there is a one-one correspondence $H$ between $\mathscr{P}$ and $\mathscr{P}^*$:

$$
H: \mathbb{P} \xrightarrow[onto]{1-1} \mathbb{P}^* 
$$

such that:

i. $H(1) = 1^*$
ii. $H(S(x)) = S^*(H(x))$ for all $x \in \mathbb{P}$ 

Such a function $H$ is called an **isomorphism** between $\mathscr{P}$ and $\mathscr{P}^*$. 
:::

::: {#exm-isomorphism}
1. Let $\mathscr{P} = (\mathbb{P}, 1, S)$ be the standard Peano system, where $\mathbb{P}$ is the set of natural numbers, "$1$" stands for the ordinary integer $1$, and $S(x) = x + 1$ for all $x \in \mathbb{P}$.

   Let $\mathscr{P}^* = (\mathbb{P}^*, 1^*, S^*)$ be the Peano system in which $\mathbb{P}^*$ is the set of even natural numbers, "$1^*$" denotes the integer $2$ and $S^*(x) = x + 2$ for all $x \in \mathbb{P}^*$.

   Then the function $H: \mathbb{P} \longrightarrow \mathbb{P}^*$ such that $H(x) = 2 \times x$ for all $x \in \mathbb{P}$ is an isomorphism between $\mathscr{P}$ and $\mathscr{P}^*$. 

   Note first that $H(1) = 2 \times 1 = 2 = 1^*$, verifying condition (i). Moreover,
   ...
   verifying (ii). It is clear that $H$ is one-one since $x \neq y \Longrightarrow 2 \times x \neq 2 \times y$ and that the range of $H$ is $\mathbb{P}^*$.

2. Again let $\mathscr{P} = (\mathbb{P}, 1, S)$ be the standard Peano system and let $\mathscr{P}^{\#} = (\mathbb{P}^{\#}, 1^{\#}, S^{\#})$, where $\mathbb{P}^{\#}$ is the set of negative integers, $1^{\#} = -1$ and $S^{\#}(u) = u - 1$ for all $u \in \mathbb{P}^{\#}$.

   Then the function $G: \mathbb{P} \longrightarrow \mathbb{P}^{\#}$ such that $G(x) = -x$ for all $x \in \mathbb{P}$ is an isomorphic between $\mathscr{P}$ and $\mathscr{P}^{\#}$.

   First, $G(1) = -1 = 1^{\#}$. In addition,
   ...
   Finally, it is obvious that $G$ is one-one since $x \neq y \Longrightarrow -x \neq -y$ and that the range of $G$ is $\mathbb{P}^{\#}$.
:::

And isomorphism $H$ between 2 Peano systems $\mathscr{P}$ and $\mathscr{P}^*$ shows that the 2 structures are essentially the same. 

Condition (i) asserts that the distinguised object $1$ of $\mathscr{P}$ corresponds to the distinguished object $1^*$ of $\mathscr{P}^*$.

Condition (ii) says that the successor operations correspond under the function $H$, this is pictured in the following diagram:

![Diagrammatic representation of condition (ii)](/images/fig-isomorphic.png){#fig-isomorphic fig-align="center" width=70%}

::: {#thm-peano-systems-isophormic}
