...
-  Let $F_{\mathcal{L}_p}$ the set of formulas of $\mathcal{L}_p$. Because $F_{\mathcal{L}_p} \subseteq \bigcup_{i \in \mathbb{N}} S_{\mathcal{L}_p}^i$ then $F_{\mathcal{L}_p}$ is denumerable.
...
2.  $\cdot$ is a $2$-place function so $\cdot+v_611$ is not a valid term for $\mathcal{L}_{NT}$, $\mathcal{L}_F$ and $\mathcal{L}_N$. Therefore $+0\cdot+v_611$ is not a valid term for those cases. 

3. $\mid1+v_30$ is a valid term for $\mathcal{L}_{NT}$, $\mathcal{L}_F$ and $\mathcal{L}_N$. It can be interpreted as $1 \mid (v_3 + 0)$.
...
8. $<v_6v_2$ is a valid term for $\mathcal{L}_O$. It can be interpreted as $v_6 < v_2$.
...
Let $B = \{ A_i \in A \mid A_i \}$ then $B$ is *pairwise disjoint* if $i \neq j$ then $A_i \cap A_j = \emptyset$.
...
Therefore if $B$ is a *pairwise disjoint* collection of sets it is *disjoint* but if $B$ is a *disjoint* collection of sets it is not necessary true that is *pairwise disjoint*.

Also there are two useful facts about unions and intersections known as *distributive laws* [@halmos_naive_2017, p. 15]:
...
We can use @exr-distributive-laws to prove these facts:
...
::: {#exr-union-intersections-halmos}
...
$$\begin{split}
  x \in C & \Longrightarrow x \in (A \cap B) \cup C \\
  & \Longrightarrow x \in A \cap (B \cup C) \\
  & \Longrightarrow x \in A
  \end{split}$$
...
The product $A \times B$ can be defined because $A \times B \subseteq \mathscr{P}(A \cup B)$. For example if $A = \{ a \}$ and $B = \{ b \}$ then $\{ \{ \{ a \}, \{ a, b \} \},  \{ \{ b \}, \{ b, a \} \} \} \subseteq \mathscr{P}(A \cup B)$.

Further for $n \geq 3$ we can define:
...
$$(A_1 \times \ldots \times A_{n-1}) = \{ (x_1, \ldots, x_n): x_1 \in A_1 \land \ldots \land x_n \in A_n \}$$

We also let,

$$X^n = \underset{n \text{ times}}{\underbrace{X \times \ldots \times X}}$$

### Difference and complement

If $A$ and $B$ are sets using @def-axiom-schema-of-specification we can define the difference between $A$ and $B$ as:

$$A - B = \{ x \in A \mid x \not \in B \}$$

Note that in this definition it is not necessary to assume $B \subseteq A$ [@halmos_naive_2017, p. 17].

Also if $A \subseteq U$ and $U \neq \emptyset$ we can define the complement of $A$ relative to $U$ as:

$$A^{'} = \{ x \in U \mid x \not \in A \}$$

We have the following useful facts about the complement [@halmos_naive_2017, p. 17]:
...
For the following deductions it is assumed that $x \in U$:
