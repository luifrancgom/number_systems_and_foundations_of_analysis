...
# See https://quarto.org/docs/computations/execution-options.html#engine-binding to specify why knitr is used
```
...
#| results: asis
#| label: tbl-true-1-4
#| tbl-cap: True tables contradiction, tautology, $\alpha$ and $\beta$
...
#| results: asis
#| label: tbl-true-9-12
#| tbl-cap: True tables AND, OR, material implication (IMPLY) and biconditional (XNOR)
...
#| tbl-cap: True tables converse implication, material nonimplication (NIMPLY), converse material nonimplication and XOR
...
#| tbl-cap: Possible true tables for $\Longrightarrow$ in a 2-valued propositional framework  
...
::: {#cor-true-values-atomic-formulas}
...
Because $u(A_n) = v(A_n)$ for every atomic formula $A_n$ in $\mathcal{L}_p$ we can take any formula $\varphi$ and apply @prp-true-values-atomic-formulas. Thefefore $v(\varphi) = u(\varphi)$ but $\varphi$ is any formula in $\mathcal{L}_p$ so it applies to every formula.  
...
Similarly, if $\Delta$ and $\Gamma$ are sets of formulas, then $\Delta$ implies $\Gamma$, written as $\Delta \models \Gamma$, if every truth assignment $v$ which satisfies $\Delta$ also satisfies
$\Gamma$.
