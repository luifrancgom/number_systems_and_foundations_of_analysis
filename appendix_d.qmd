...
However, by @def-onto-function $F$ is not onto because $\mathscr{D}(F) = \emptyset$ so $\exists x(x \in \emptyset \land F(x) = y)$ is false so $\forall y (y \in P_1 \Longrightarrow \exists x(x \in \emptyset \land F(x) = y))$ is false.
...
- *Case 2* $G(k + 1) \neq k + 1$: Then $k + 1 \not \in A$ or $k + 1 \in A$
    ...
    - If $k + 1 \in A$ then there is some $i \in P_n$ such that $G(k + 1) = i$ and some $j \in P_n$ such that $G(j) = k + 1$. Therefore we can define a function $F: P_k \xrightarrow[onto]{1-1} A - \{k + 1\}$ as:
